...
const Gallery = () => {
  ...
  // SAFE DATA ACCESS
  const galleryImages = state?.data?.gallery || [];

  const [selectedImage, setSelectedImage] = useState<string | null>(null);
  const [selectedCategory, setSelectedCategory] = useState('All');

  const defaultCategories = ['All', 'General', 'Event', 'Festivals', 'Activities', 'Facilities'];

  // Generate categories safely
  ...
  const categoriesToShow =
    uniqueCategories.length > 1 ? uniqueCategories : defaultCategories;
            ...
            {(filteredImages || []).map((image) => (
              ...
              >
                ...
                <div className="h-48 overflow-hidden">
                  ...
                  <img
                    src={image?.url}
                    alt={image?.caption || image?.altText || "Gallery Image"}
